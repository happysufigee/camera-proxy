...
void RemixLightingManager::SubmitManagedLight(ManagedLight& candidate) {
    ...
    if (!m_settings.disableDeduplication) {
        ...
        if (it != m_activeLights.end()) {
            ...
            m_remix.UpdateLight(it->second.handle, desc, candidate.signatureHash);
            it->second.drawCounter = 1;
            return;
        }
    }
    ...
    candidate.handle = m_remix.CreateLight(desc, candidate.signatureHash);
